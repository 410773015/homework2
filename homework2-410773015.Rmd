...
commit1<-c("commit")
aL<-c(1:30)
names(commitCheck)<-str_c(commit1,aL)
names(commitCheck)
commitCheck
```
...
```{r ans25}
allcommitDatesTW<-ymd_hms(allcommitDates,tz="Asia/Taipei")
allcommitDatesTW
```
...
```{r ans26}
bL<-str_sub(allcommitDatesTW,12,13)
cL<-str_sub(allcommitDatesTW,15,16)
dL<-str_sub(allcommitDatesTW,18,19)
logicwithinTimeFrame<-as.numeric(str_c(bL,cL,dL))%in%c(43000:63000)
...
```{r ans34}
transcriptPartialDataEcon<-list("學號"=transcriptData[["學號"]][logicFromEcon],"學分數"=transcriptData[["學分數"]][logicFromEcon],"必選修類別（必∕選∕通）"=transcriptData[["必選修類別（必∕選∕通）"]][logicFromEcon])
